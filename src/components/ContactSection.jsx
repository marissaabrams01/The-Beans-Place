// ContactSection — "Get In Touch": message form on the left, tilting contact
// cards and social links on the right, over an animated background.

// useState -> re-renders on change; useEffect -> runs after render;
// useRef -> holds a DOM reference across renders
import { useState, useEffect, useRef } from "react";
// useMotionValue -> changes WITHOUT re-rendering (ideal for mouse position);
...
// Input is the default export; Textarea is an extra export from the same file
import Input, { Textarea } from "./ui/Input";
import Button from "./ui/Button";

// Data for the four "ways to reach us" cards.
// name/detail -> card text        icon -> inline SVG
// gradient    -> icon tile colour accentColor -> the bar down the card's edge
// action/href -> where it links
const contactChannels = [
    {
        name: "Visit Our Roastery",
        // "currentColor" makes the icon inherit the surrounding text colour
        icon: (
    ...
    },
    {
        ...
        detail: "(303) 555-BEAN",
        action: "Call Now",
        href: "tel:+13035552326" // phones can dial straight from the page
    }
];

/* ===== Tilt card ===== */
// Wrapper that tilts its contents in 3D toward the mouse. Local to this file.
...
function TiltCard({ children, className, href, target, rel }) {
    ...
    // Fractions -> rotation angles, smoothed by a spring (higher damping settles
    // faster). rotateX is reversed [6, -6] so moving the mouse down tips the top
    // of the card away from you — that's what sells the 3D.
    const rotateX = useSpring(useTransform(y, [-0.5, 0.5], [6, -6]), { stiffness: 200, damping: 20 });
    const rotateY = useSpring(useTransform(x, [-0.5, 0.5], [-6, 6]), { stiffness: 200, damping: 20 });

    function handleMouse(e) {
        const rect = ref.current.getBoundingClientRect();
        // Window coords -> card-relative -> 0-to-1 fraction -> recentred on 0
...
function ContactFormInline() {
    // All four fields in one object. These keys must match the "name" props on
    // the Inputs below — that's what makes handleChange work.
    const [formData, setFormData] = useState({ name: "", email: "", subject: "", message: "" });
    const [status, setStatus] = useState(null); // null | 'sending' | 'sent' | 'error'
    // Fields the user has visited and left. Errors only show for these, so the
    // form doesn't turn red before they've typed anything.
    ...
    // "blur" fires on leaving a field — mark it visited
    const handleBlur = (e) => {
        ...
        <motion.form
            ...
            <div className="contact-form-grid">
                <Input
                    ...
                    // value + onChange = a "controlled input": React holds the
                    // text and the box always shows what state says
                    value={formData.name}
            ...
            <div className="mt-6 flex items-center gap-4">
                <Button
                    ...
                    {/* Label comes from `status`: sending -> spinner,
                        sent -> tick, otherwise the normal text */}
                    {status === "sending" ? (
                        <span className="flex items-center gap-2">
                            {/* animate-spin (Tailwind) rotates this forever */}
...
export default function ContactSection() {
    ...
    return (
        <div className="contact-section-wrap" ref={sectionRef}>
            {/* Decorative only — three blurred orbs and a faint grid, positioned
                and animated by CSS. Empty divs: they exist to be styled. */}
            ...
            <div className="contact-inner">
                ...
                <ScrollReveal animation="fadeUp" className="contact-header">
                    ...
                    {/* "52ch" ≈ 52 characters per line — a comfortable measure */}
                        ...
                        <StaggerContainer className="contact-info-cards" staggerDelay={0.1}>
                            {contactChannels.map((channel) => (
                                ...
                                <StaggerItem key={channel.name} animation="fadeUp">
                                    <TiltCard
                                        href={channel.href}
                                        // Only external ("http") links open in a new tab;
                                        // undefined leaves the attribute off entirely, so
                                        // mailto:/tel:/#contact links stay in this tab.
                                        target={channel.href.startsWith("http") ? "_blank" : undefined}
                                        // rel is a security must on any target="_blank" link —
                                        // it stops the new page getting a handle back to ours
                                        rel={channel.href.startsWith("http") ? "noopener noreferrer" : undefined}
                                        className="contact-card-link">
                                        <div className="contact-card">
                                            {/* Edge bar — inline style because the
                                                colour comes from the data */}
                                            <div
                                                className="contact-card-accent"
                                                style={{ background: channel.accentColor }}
                                            />
                                            <div className="contact-card-content">
                                                {/* Fixed classes + this channel's gradient,
                                                    fading to the bottom-right */}
            ...
            {/* Glow trailing the cursor (desktop only). Subtracting 192px — half
                its width — puts its centre under the pointer instead of its
                corner. Inline style, since the position changes constantly. */}
            <div
